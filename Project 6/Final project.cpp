...
const float		EYEDISTFACTOR = 1.0f;	// distance from look to eye = this factor * planet radius
const float		ZFAR = 1000000.0f;
const float		FOVDEG =     90.0f;

enum views
{
	OUTSIDEVIEW,   EARTHVIEW,   MOONVIEW,   SWISSVIEW, CORVALLISVIEW
};
enum views	NowView;  
...
const float ONE_FULL_TURN                        =      F_2_PI;	// this is 2π instead of 360° because glm uses radians
...
const char *WINDOWTITLE = "Solar Eclipse Chakradhar Reddi's project";
...
const float EARTH_SPIN_TIME_DAYS = TS*0.9971f * 4.f;
...
const float MOON_RADIUS_MILES = PRS*2500.6f;
...
bool		DoLighting;
GLuint		SphereDL, SunDL, EarthDL, MoonDL;
...
LatLngToXYZ(float lat, float lng, float rad )
{
	...
	xyz.x = -rad * xz * sinf(lng);
	xyz.y =  rad * sinf(lat);
	xyz.z =  rad * xz * cosf(lng);
	return xyz;
}
...
SetViewingFromLatLng(float eyeLat, float eyeLng, float lookLat, float lookLng, float rad,   glm::vec4 * eyep, glm::vec4 * lookp, glm::vec4 * upp, float *znearp )
{
	...
	glm::vec3 perpComponent = upVec * glm::dot( eyeToLook, upVec );
	...
	float disc = SQR(B) - 4.f*A*C;
	if (disc < 0.)
	{
		fprintf(stderr, "disc = %8.3f\n", disc);
		znear = 1.0f;
	}
	else
	{
		...
		if(n2 < znear)
			znear = n2;
		if (znear < 0.)
			fprintf(stderr, "znear = %8.3f\n", znear);
	}

	//fprintf(stderr, "look = (%8.3f,%8.3f,%8.3f) , eyeprime = (%8.3f,%8.3f,%8.3f) , znear = %8.3f\n",
		//look.x, look.y, look.z, eyeprime.x, eyeprime.y, eyeprime.z, znear);
...
MakeEarthMatrix( )
{
	float earthSpinAngle  =  ONE_FULL_TURN * Time * TimeScale / EARTH_SPIN_TIME_SECONDS;
...
/* 4. */	glm::mat4 erorbity  = glm::rotate(      identity, earthOrbitAngle, yaxis );
/* 3. */	glm::mat4 etransx   = glm::translate(   identity, glm::vec3( EARTH_ORBITAL_RADIUS_MILES, 0., 0. ) );
...
etransx = glm::translate(identity,  glm::vec3(EARTH_ORBITAL_RADIUS_MILES * cosf(earthOrbitAngle), 0., EARTH_ORBITAL_RADIUS_MILES * sinf(earthOrbitAngle) ) );
erorbity = identity;
return erorbity * etransx * etilt * erspiny;		// 4 * 3 * 2 * 1
}
...
Display( )
{
	...
	if (NowProjection == ORTHO)
		glOrtho(-2.f, 2.f, -2.f, 2.f, znear, 1000.f);
	else
		gluPerspective(70.f, 1.f, znear, 1000.f);
...
InitGraphics( )
{
	...
	file = (char*)"worldtex.bmp";	// mjb "earth.bmp";
...
Keyboard( unsigned char c, int x, int y )
{
	...
	switch( c )
	{
		...
		case 's':
			// Enable the Sun's light to restore normal lighting
			glEnable(GL_LIGHT0);
			printf("Sun's light enabled\n");
			break;
