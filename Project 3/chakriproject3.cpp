...
const char* WINDOWTITLE = "OpenGL / GLUT Sample -- Chakradhar reddi's  Project03 Graphics";
...
GLuint  cowDL;                  // cow display list
GLuint  carDL;                  // car display list
GLuint  skeletonDL;                   // skeleton display list
GLuint	BoxList;
...
float	ltX, ltY, ltZ;	    // light source coordinates
float   r, g, b;
GLfloat ltcolor[] = { 1.0f, 1.0f, 1.0f, 1.0f }; // White color;
bool     lightstate = 1;
int     shadetype = 0;
...
Display()
{
	...
	if (lightstate != 0)
		SetPointLight(GL_LIGHT0, ltX, ltY, ltZ, ltcolor[0], ltcolor[1], ltcolor[2]);
	else
		SetSpotLight(GL_LIGHT0, ltX, ltY, ltZ, 0.f, -1.f, 0.f, ltcolor[0], ltcolor[1], ltcolor[2]);
	...
	glTranslatef(ltX, ltY, ltZ);
	glCallList(ltsource);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(10.f, 3.f, 0.f);
	glColor3f(4.f, 0.0f, 1.0f);
	glCallList(cowDL);
	glPopMatrix();
	...
	glPushMatrix();
	glTranslatef(0.f, 3.f, -11.f);
	glColor3f(0.f, 1.0f, 0.f);
	glCallList(carDL);
	glPopMatrix();

	glPushMatrix();
	glTranslatef(-12.f, 0.2f, 0.f);
...
InitLists()
{
	...
	SetMaterial(0.5f, 0.0f, 1.0f, 30.f);
...
GLfloat pointLightPosition[] = { 0.0f, 5.0f, 0.0f, 1.0f };
...
Keyboard(unsigned char c, int x, int y)
{
	...
	case 'w':
		ltcolor[0] = 1.0f; // Red
		ltcolor[1] = 1.0f; // Green
		ltcolor[2] = 1.0f; // Blue
		break;
	case 'R':
	case 'r':
		ltcolor[0] = 1.0f; // Red
		ltcolor[1] = 0.0f; // Green
		ltcolor[2] = 0.0f; // Blue
		break;
	case 'G':
	case 'g':
	...
	if (lightstate) {
		if (lightType == 0) {
			glLightfv(GL_LIGHT0, GL_DIFFUSE, ltcolor); // Set the light color
			glLightfv(GL_LIGHT0, GL_SPECULAR, ltcolor); // Set specular color if needed
			glLightfv(GL_LIGHT0, GL_AMBIENT, ambientLightColor); // Set ambient light if needed
			glLightf(GL_LIGHT0, GL_SPOT_CUTOFF, 180.0f);
			...
			glLightfv(GL_LIGHT0, GL_SPECULAR, ltcolor); // Set specular color if needed
			glLightfv(GL_LIGHT0, GL_AMBIENT, ambientLightColor); // Set ambient light if needed
			glLightf(GL_LIGHT0, GL_SPOT_CUTOFF, 30.0f); // Adjust for spotlight
